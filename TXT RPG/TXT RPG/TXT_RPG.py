...
def GameOver (message):
    if config.char.starv == 0:
        ...
        bot.send_message(message.chat.id, 'Вы упали в обморок и сдохли от голода. КОНЕЦ ИГРЫ!')
    elif config.char.starv >=100:
        config.char.starv = 100

    elif config.char.sleep == 100:
        config.isGameStart = False
        bot.send_message(message.chat.id, 'Соснул на паре. Отчислили. КОНЕЦ ИГРЫ!')
        ...
        config.isGameStart = False
        bot.send_message(message.chat.id,'Вы осознали,что этот универ - шарага и отчислились самостоятельно. КОНЕЦ ИГРЫ!')
...
def help_message(message):
   if config.isGameStart:
    if config.inWork:
        ...
        config.inWork=True
        bot.send_message(message.chat.id,'Вы - вейпер, как не стыдно то в 2к19?')
        time.sleep(5)
        config.inWork = False
        bot.send_message(message.chat.id,'https://www.aziaminvatat.ro/images/aai-images/aai-9510-2.jpg')
        bot.send_message(message.chat.id, 'Вы поболтали со знакомыми 30минут и получили -2 к стрессу. -0,5Ч +0И +1У -2C -2С')
